...
trait Tables {
  ...
  case class TodoRow(id: Int, title: String, text: String, status: Boolean)
  /** GetResult implicit for fetching TodoRow objects using plain SQL queries */
  implicit def GetResultTodoRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean]): GR[TodoRow] = GR{
    prs => import prs._
    TodoRow.tupled((<<[Int], <<[String], <<[String], <<[Boolean]))
  }
  ...
  class Todo(_tableTag: Tag) extends profile.api.Table[TodoRow](_tableTag, "todo") {
    def * = (id, title, text, status) <> (TodoRow.tupled, TodoRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(title), Rep.Some(text), Rep.Some(status)).shaped.<>({r=>import r._; _1.map(_=> TodoRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
